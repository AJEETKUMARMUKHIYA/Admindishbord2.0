...
doc.text("info@packyatra.com", 175, 38, { align: "right" });
  ...
  ======================= */
   y += 35;
  doc.setFontSize(18);
  doc.text(" Booking Confirmation", 100, y, { align: "center" });
  y += 5;
...
autoTable(doc, {
  startY: y,
    ...
    body: [
    ["Invoice No", bookingData.invoice.invoiceNumber  || "-", "Pickup Date", bookingData.invoice.pickupDate || "-"],
    ["Name", bookingData.invoice.customerName || "-", "Pickup Time", bookingData.invoice.slotTime || "-"],
    ["Mobile No", bookingData.invoice.phone || "-", "Delivery ETA", "-"],
    ["Moving Type", "Household", "Total Volume(CFT)", bookingData.invoice.totalVolume||"-"],
  ],
  theme: "grid",
...
goods from ${bookingData.invoice.fromAddress || "______"} to ${bookingData.invoice. toAddress || "______"}.
We are pleased to quote our rates for the same as under:`;
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ["Car Transportation Charges",  bookingData.invoice.totalAmount?.car || "-"],
      ["GRAND TOTAL", bookingData.invoice.totalAmount || "-"],
    ],
    theme: "grid",
